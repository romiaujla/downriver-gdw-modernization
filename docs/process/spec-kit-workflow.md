...
3. Before creating a new Jira-linked branch, run `git checkout main && git pull`, then create the Jira-linked branch; if the Jira-linked branch already exists, validate and use that existing branch.
...
- `--audit GRA-<id>` audits the active implementation against Jira scope, acceptance criteria, validation evidence, and out-of-scope changes before merge or handoff.
...
- `--sync-to-jira GRA-<id>` pushes branch-derived implementation notes, validation evidence, and accepted scope changes back into Jira without rewriting unrelated issue sections.
- `--sync-from-jira GRA-<id>` pulls Jira changes into an active branch only while that branch remains in flight; if the prior Jira-linked branch already merged to `main`, create a follow-up Jira issue instead of reopening the merged branch scope.
